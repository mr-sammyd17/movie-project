...
export default function Home() {
  const [apiMovieData, setApiMovieData] = useState([]) //Variable used to store the movie data when it arrives from the API call
  ...
  const [searchText, setSearchText] = useState("") //Variable used to store what is typed in the search box by the user
  ...
  return (
    ...
    <div>
      ...
      <Head>
        ...
        <h2 style={{margin: 'unset', color : 'white'}}><u>This page lists the top {filteredMovies.length} movies according to IMDb</u></h2>
      <Gallery movieData={filteredMovies} pageCount={count}/>
      <div style = {{background: 'grey', display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between'}}>
        <button onClick={Previous}>Previous</button> 
        <h5>You are seeing the movies in view {count} of {filteredMovies.length/5}</h5>
        <button onClick={Next}>Next</button>
      </div>
      <div style = {{background: 'pink', display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between'}}>
        <input type = "text" placeholder="Search for movies here" value={searchText} onChange={handleSearchChange}></input>
      </div>
    </div>
  )
}
